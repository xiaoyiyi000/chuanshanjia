package com.aidex.common.utils.poi;
...
import com.aidex.common.utils.file.FileTypeUtils;
...
{
    private static final Logger log = LoggerFactory.getLogger(com.aidex.common.utils.poi.ExcelUtil.class);
     ...
     * @return
     */
    public R exportErrExcel(List<T> list, String sheetName)
    {
        this.initErr(list, sheetName, Type.EXPORT);
        return exportExcel();
    }
    public void initErr(List<T> list, String sheetName, Type type)
    {
        if (list == null)
        {
            list = new ArrayList<T>();
     ...
     * 得到所有定义字段
     */
    private void createErrExcelField()
    {
        this.fields = new ArrayList<Object[]>();
        List<Field> tempFields = new ArrayList<>();
        tempFields.addAll(Arrays.asList(clazz.getSuperclass().getDeclaredFields()));
        tempFields.addAll(Arrays.asList(clazz.getDeclaredFields()));
        //记录错误信息列对应的field和Excel
        Field importErrInfoField = null;
        Excel importErrInfoExcel= null;
        for (Field field : tempFields)
        {
            //默认错误信息列不导出
            if("importErrInfo".equals(field.getName())){
                    importErrInfoField = field;
                    importErrInfoExcel = field.getAnnotation(Excel.class);
                continue;
            }
            // 单注解
        ...
        //记录错误信息列对应的field和Excel添加到Excel最后一列
        putToField(importErrInfoField, importErrInfoExcel);
    ...
    {
        String downloadPath = AiDexConfig.getDownloadPath() + filename;
